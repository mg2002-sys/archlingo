 ...
 * Handles:
 *  - 500-Stage Winding Duolingo Skill Path Map with Phase Headers
 *  - Instant "Jump to Stage # (1–500)" Navigation & Phase Filtering
 *  - Dedicated "🐣 New Words Explained Simply (Zero Jargon)" Glossary Card on Every Stage
 *  - Deep Concept View (Glossary + 3 Rich Concept Cards + Live Interactive Simulator)
...
class ArchLingoApp {
  ...
  loadState() {
    const saved = localStorage.getItem("archlingo_500_state_v1");
    if (saved) {
      try {
  ...
  saveState() {
    localStorage.setItem("archlingo_500_state_v1", JSON.stringify(this.state));
  ...
  updateTopNavStats() {
    ...
    const totalStages = this.stages.length || 500;
  ...
  renderSectionFilterBar() {
    ...
    container.innerHTML = `
      <div class="flex flex-wrap items-center justify-between gap-3 bg-white dark:bg-slate-800 p-3.5 rounded-2xl border-2 border-slate-200 dark:border-slate-700 shadow-sm">
        <div class="flex items-center gap-2 flex-1 min-w-[230px]">
        ...
        <div class="flex items-center gap-2 flex-1 min-w-[200px]">
          <span class="text-lg">🔍</span>
          <input id="stage-search-input" type="text" placeholder="Search ${totalStages} stages (e.g. DNS, Hashing, Cache)..."
      ...
      sectionStages.forEach((stage, idx) => {
        ...
        html += `
          <div id="stage-node-${stage.id}" class="relative flex flex-col items-center group"
            ...
            <div class="mt-1.5 text-center max-w-[190px]">
  ...
  renderConceptTabHtml(stage) {
    ...
    const glossaryHtml = beginnerGlossary
      .map(
        (item) => `
        <div class="bg-white dark:bg-slate-800 rounded-2xl p-4 border-2 border-amber-200 dark:border-slate-700 shadow-sm">
          <div class="font-black text-sm text-emerald-700 dark:text-emerald-400 flex items-center gap-1.5">
            <span>📘</span>
            <span>${item.term}</span>
          </div>
          <p class="text-xs font-bold text-slate-700 dark:text-slate-200 mt-1 leading-relaxed">
            ${item.definition}
          </p>
          <div class="mt-2 text-xs font-bold text-amber-900 dark:text-amber-200 bg-amber-100/80 dark:bg-amber-950/50 px-3 py-2 rounded-xl border border-amber-300 dark:border-amber-700">
            💡 <span class="font-black">Everyday Analogy:</span> ${item.analogy}
          </div>
        </div>
      `
      )
        ...
        <div class="bg-gradient-to-r from-emerald-500/10 via-sky-500/10 to-purple-500/10 border-2 border-emerald-500/30 rounded-2xl p-4 flex items-start gap-3">
          ...
          <div>
            <div class="font-black text-sm text-slate-900 dark:text-white">
              Archie's Zero-Jargon Concept Guide — Step-by-Step From Beginner to Pro!
            </div>
            <p class="text-xs font-bold text-slate-600 dark:text-slate-300 mt-0.5">
              Start with the <strong>🐣 New Words Explained Simply</strong> glossary below, explore how ${stage.company} uses this pattern, and test live traffic in the Simulator Lab!
            </p>
          </div>
        </div>

        <!-- 🐣 DEDICATED BEGINNER WORD GLOSSARY & REAL-LIFE ANALOGY CARD -->
        <div class="duo-card p-5 border-2 border-amber-400 bg-amber-50/40 dark:bg-amber-950/20">
          <div class="flex flex-wrap items-center justify-between gap-2 mb-3">
            <span class="text-xs font-black uppercase tracking-wider px-3 py-1 rounded-xl bg-amber-400 text-amber-950 shadow-sm">
              🐣 New Words Explained Simply (Zero Jargon)
            </span>
            <span class="text-xs font-extrabold text-amber-700 dark:text-amber-300">
              Real-Life Everyday Analogies
            </span>
          </div>
        ...
        <div class="duo-card p-5">
          <div class="flex items-center justify-between mb-3">
            <span class="text-xs font-black uppercase tracking-wider px-2.5 py-1 rounded-lg bg-emerald-100 dark:bg-emerald-950 text-emerald-800 dark:text-emerald-300">
              📐 Card 1 of 3 • Step-by-Step How It Works & Formula
            </span>
        ...
        <div class="duo-card p-5">
          <div class="flex items-center justify-between mb-3">
            <span class="text-xs font-black uppercase tracking-wider px-2.5 py-1 rounded-lg bg-sky-100 dark:bg-sky-950 text-sky-800 dark:text-sky-300">
              💻 Card 2 of 3 • Under the Hood Code (${codeCard.language.toUpperCase()})
            </span>
          ...
          <div class="mt-3 flex items-center gap-2 text-xs font-extrabold text-sky-700 dark:text-sky-300 bg-sky-50 dark:bg-sky-950/50 px-3 py-2 rounded-xl border border-sky-200 dark:border-sky-800">
            <span>💡</span>
            <span>Takeaway: ${codeCard.takeaway}</span>
          </div>
        </div>
        ...
        <div class="duo-card p-5">
          <div class="flex items-center justify-between mb-3">
            <span class="text-xs font-black uppercase tracking-wider px-2.5 py-1 rounded-lg bg-purple-100 dark:bg-purple-950 text-purple-800 dark:text-purple-300">
              🏢 Card 3 of 3 • Real-World Story: ${caseStudyCard.company}
            </span>
            <span class="text-xs font-extrabold text-emerald-600 dark:text-emerald-400">Verified Impact</span>
          </div>

          <div class="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs">
            <div class="bg-rose-50 dark:bg-rose-950/40 border border-rose-200 dark:border-rose-800 rounded-xl p-3.5">
              <div class="font-black text-rose-800 dark:text-rose-300 uppercase mb-1">🚨 Challenge</div>
            ...
            <div class="bg-sky-50 dark:bg-sky-950/40 border border-sky-200 dark:border-sky-800 rounded-xl p-3.5">
              <div class="font-black text-sky-800 dark:text-sky-300 uppercase mb-1">🛠️ Solution</div>
            ...
            <div class="bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-800 rounded-xl p-3.5">
              <div class="font-black text-emerald-800 dark:text-emerald-300 uppercase mb-1">📈 Result</div>
            ...
            <div class="p-4 rounded-2xl border-2 ${
              ...
              <div>
                <div class="font-black text-base flex items-center gap-2">
                  <span>${this.selectedOptionIdx === correctIdx ? "✅ Awesome! Correct!" : "❌ Keep learning! (-1 ❤️)"}</span>
                </div>
                <p class="text-xs font-bold mt-1 opacity-90">${q.explanation}</p>
              </div>
              <button
                onclick="window.app.nextQuizQuestion()"
                class="${this.selectedOptionIdx === correctIdx ? "duo-btn-green" : "duo-btn-blue"} px-6 py-3 text-sm whitespace-nowrap"
              >
  ...
  completeStage(stage) {
    ...
    const nextStageId = Math.min(totalStages, stage.id + 1);
    if (stage.id >= this.state.unlockedStage && this.state.unlockedStage < totalStages) {
      ...
      <div class="text-center space-y-5 py-3">
        ...
        <div class="grid grid-cols-3 gap-3">
          ...
          <div class="duo-card p-3 bg-sky-50 dark:bg-sky-950/40 border-sky-300">
            ...
            <div class="text-xl font-black text-sky-600 mt-0.5">${this.state.completedStages.length}/${totalStages}</div>
          </div>
        </div>
